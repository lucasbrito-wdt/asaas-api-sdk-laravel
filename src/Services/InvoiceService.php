...
use Asaas\Laravel\Http\Requests\Invoice\ListInvoicesRequest;
use Asaas\Laravel\Models\InvoiceList;
...
class InvoiceService extends BaseService
{
    ...
    public function list(array $params = []): InvoiceList
    {
        $this->addErrorMapping(400, ErrorResponseDtoException::class);
        $query = array_filter(ListInvoicesRequest::validate($params), fn ($v) => $v !== null);
        $builder = new RequestBuilder('v3/invoices', 'GET');
        foreach ($query as $key => $value) {
            $builder->setOptionalQueryParameter($key, $value);
        }
        $request = $this->buildRequest($builder);
        /** @var InvoiceList $result */
        $result = $this->executeAndMap($request, InvoiceList::class);
        return $result;
    }
}
