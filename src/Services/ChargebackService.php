...
use Asaas\Laravel\Http\Requests\Chargeback\ListChargebacksRequest;
use Asaas\Laravel\Models\ChargebackList;
...
class ChargebackService extends BaseService
{
    ...
    public function list(array $params = []): ChargebackList
    {
        $this->addErrorMapping(400, ErrorResponseDtoException::class);
        $query = array_filter(ListChargebacksRequest::validate($params), fn ($v) => $v !== null);
        $builder = new RequestBuilder('v3/chargebacks', 'GET');
        foreach ($query as $key => $value) {
            $builder->setOptionalQueryParameter($key, $value);
        }
        $request = $this->buildRequest($builder);
        /** @var ChargebackList $result */
        $result = $this->executeAndMap($request, ChargebackList::class);
        return $result;
    }
}
