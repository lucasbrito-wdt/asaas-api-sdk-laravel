...
use Asaas\Laravel\Http\Requests\Checkout\ListCheckoutRequest;
use Asaas\Laravel\Models\CheckoutList;
...
class CheckoutService extends BaseService
{
    ...
    public function list(array $params = []): CheckoutList
    {
        $this->addErrorMapping(400, ErrorResponseDtoException::class);
        $query = array_filter(ListCheckoutRequest::validate($params), fn ($v) => $v !== null);
        $builder = new RequestBuilder('v3/checkout', 'GET');
        foreach ($query as $key => $value) {
            $builder->setOptionalQueryParameter($key, $value);
        }
        $request = $this->buildRequest($builder);
        /** @var CheckoutList $result */
        $result = $this->executeAndMap($request, CheckoutList::class);
        return $result;
    }
}
